...
namespace YAMLImportTester
{
    class Program
    {
        ...
        void DoStuff()
        {
            ...
            while (look != 0 || line == "test" || line == "code")
            {
                bool found = false;
                if (look != oldLook)
                {
                    for (int i = 0; i <= items.Length - 2 && found == false; ++i)
                    {
                ...
                {
                    Console.WriteLine("Interation start:");
                    string user = Console.ReadLine();
                    int search = Int32.Parse(user);

                    Console.WriteLine("name comparison term: ");
                    ...
                    foreach(var item in items){
                        if (item != null)
                        {
                            ...
                            if (name != null && name != "")
                            {
                                ...
                                if (test)
                                {
                                    Console.WriteLine("blacklist[" + search + "] = " + item.getBlueprintID());
                                    itemList += "blacklist[" + search + "] = " + item.getBlueprintID() + ";\n";
                                    ++search;
                                }
                            }
                        }
        ...
        void YdnSetName(string fileLocation, string language)
        {
            ...
            foreach (var entry in map.Children)
            {
                ...
                for (int i = 0; i < items.Length - 1 && found == false; ++i){
